window.addEventListener('DOMContentLoaded', function(){
    ...
    var createScene = function () {
        ...
        //vect = draw_sqare(40);
        // console.log(vect);
        /*var shape = BABYLON.MeshBuilder.CreatePolygon("square", {shape: draw(rad, "sqaure"), depth:1}, scene);
        shape.position = new BABYLON.Vector3(0, north_s.position.y, 0);
        var mt = new BABYLON.StandardMaterial("line_mat", scene);
        mt.diffuseColor = new BABYLON.Color3(0.5, 0, 0);
        shape.material = mt;
        shape.isVisible = true;
        shape.alphaIndex = 0.6;*/
    ...
    square.onIsCheckedChangedObservable.add(function(state){
        if(state){
            shape_d.dispose();
            shape = "square";
            //shape_d = BABYLON.MeshBuilder.CreatePolygon("shape", {shape: draw(rad, shape), depth:1, updatable:true}, scene);
            draw(rad, shape);
            shape_d.isVisible = true;
        }
    });
    ...
    x_axis.onIsCheckedChangedObservable.add(function(state){
        if(state){
            //shape_d.rotateAround(new BABYLON.Vector3(1, 0, 0));
